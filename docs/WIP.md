...
| Recipe library | **24 recipe dirs** via `_recipe_dirs`. **21** declare `native_octane_verified=true`; **23** have `octane-preview.png`; **1** lacks a preview (`earth-moon-space`). Honest flag gaps: `birthday-cake` (preview present), `helicoid-spiral` (preview present), `earth-moon-space` (no preview). |
...
1. **A2 — Recipe verification reconciliation** (LOW effort / HIGH integrity): close the three honest flag gaps. `earth-moon-space` needs a fresh native render and preview; `birthday-cake` and `helicoid-spiral` have previews but need pixel/vision/result-metadata review before flipping `native_octane_verified=true`. *First step:* run the recipe verifier per slug with `copy_back=True`, inspect, and append `docs/recipe-book.md` notes for any bridge/material lesson.
