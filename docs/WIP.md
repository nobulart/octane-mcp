...
| Area | State |
|------|-------|
| Repo | `main` = `f9e58a7` (HEAD; tree **clean**). Recent: lexical intent graph (e8b89f6), WP6 chess recipes (b29cd45), `swap_geometry` tool (H2, 7d30b26), bridge canonical-node fix (888329c), WP8 math-surface gap fix (3bbdc62) |
| Tests | **319 ran / 0 failures / 3 skipped** (offline `python -m unittest discover -s tests`, project venv). Green. 3 skipped (live render ×1; `geo` extra ×2). `compileall src` clean; `doctor --json` clean (with `OCTANEX_MCP_WORKSPACE` override). |
| Recipe library | **22 recipe dirs; 21 `native_octane_verified=true` (each carries an `octane-preview.png`); 1 honestly-declared pending.** Pending: `earth-moon-space` (no PNG on disk). Ground truth via `_recipe_dirs` + `scene.json` + pixel QA on the committed PNG. `helicoid-spiral` PNG now verifies (was a blank frame, re-rendered). The old "20/22 / 16-recipe gap" lines were stale — library is now 21/22. |
...
**Bottom line:** reliability + core mechanics are solid, and recipe-library honesty is now explicit rather than inflated: **21/22 verified**, with 1 live-render closure item still open. The remaining open work is live closure + surface area/polish.
