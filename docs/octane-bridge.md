...
- A standalone relaunch+click driver lives at `scripts/relaunch_octane_oneshot.applescript` (quit Octane → relaunch → click `Script > hermes_bridge_oneshot.generated`). Verified live: with TCC granted + Hermes restarted it returns `clicked hermes_bridge_oneshot.generated via Script` (exit 0). It cannot perform the manual "Hermes Render Target" re-select, which remains a required human step after the click.
