...
## A2 — pointcloud pipeline merged; earth-hemisphere later verified
- **Outcome:** superseded by v4.1 native render verification
- **Recorded:** 2026-07-13
...
- Original merge-time state: `verify_recipe_library(dry_run=True)` reported `total=30`, `contract_ok=28`, `contract_failed=2`; the intentional failures were `earth-hemisphere` (new point-cloud recipe, no native preview yet) and `earth-moon-space` (existing no-preview gap).
- Superseding 2026-07-13 state: v4.1 produced and committed a native `octane-preview.png`; `earth-hemisphere` now passes pixel QA and is marked `native_octane_verified=true`. `earth-moon-space` remains the sole no-preview contract failure.
### Follow-ups
- For higher-density production imagery, regenerate locally with a larger `--density` and do not commit multi-GB OBJ outputs.
---
