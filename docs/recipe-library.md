...
| [Menger Sponge](../examples/recipes/menger-sponge/README.md) | Mathematics/geometry | `menger-sponge` | Recursive fractal cube (3D Sierpinski analogue): 8,000 solid subcubes at depth 3, single blue glossy material, flat-shaded. |
...
- `simulation-frame-strip` ✅ landed (offline-verified, contract-clean, **native-promoted**). Defines the
  ...
  advection-diffusion pulse (no external simulator), so it is the template every
  future per-frame export adapter (later Phase C / simulator exports) maps onto.
