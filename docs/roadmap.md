...
- **Recipe library ground truth:** `_recipe_dirs(examples/recipes)` reports **26 recipe dirs** (post-`ancient-temple` merge). **24** declare `native_octane_verified=true`; **25** carry `octane-preview.png`; **1** lacks a preview (`earth-moon-space`). A2 reconciliation (2026-07-12) closed the two metadata/QA gaps: `birthday-cake` promoted to `native_verified` (pixel-QA + vision), `helicoid-spiral` REJECTED by pixel-QA (blank flat field) and stays `false`. The single remaining honest gap is `earth-moon-space` (flag false, no preview — a live render gap, not a metadata fix).
