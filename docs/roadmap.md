...
## Status snapshot (2026-07-13 — steward review, HEAD `196185c`)

Live-checked 2026-07-13 from `main` at `196185c` (`feat(geo): EGM2008 geoid heightfield recipe + gen_geo_displacement/queue_geo_surface tooling`). Fresh evidence; docs' previous `296e7f9` snapshot is **12 commits stale**.

- **Repo:** `main` = `196185c`, tracking `origin/main`; 12 commits ahead of the docs' claimed `296e7f9`. Recent: geo heightfield recipe (top), EGM2008 geoid OBJ, gallery surface pipeline (schwarz_p/d/lidinoid/neovius/gyroid/SchwarzH), desk fan recipe, osascript control toolkit, material pins + texture wiring, scene sanity gates, and pre-render reports.
- **Tests:** `PYTHONPATH= uv run python -m unittest discover -s tests` ran **381 tests / 1 failure / 3 skipped**. Benchmark subset: **14 tests / 0 failures / 1 skipped** (`live_tier1_tier2` skips without `OCTANEX_LIVE=1`). `compileall src` passed. The **one failure** is the **pre-existing** `test_lua_bridge_parity::test_scene_handler_semantics_match_between_one_shot_and_persistent_bridges` (`wait_for_render_ready` body drift between templates — not a regression). **+24 new** WP7 pre-render sanity tests.
- **Doctor / bridge:** `octanex-mcp doctor --json` returned `ok: true`. Octane X running; persistent bridge `status=processed`, `render_stage=ready`, `samples_done=1200`, `samples_target=1200`, `last_event=save_preview preview saved .../egm2008_octane-preview.png`.
- **Recipe library ground truth:** `_recipe_dirs(examples/recipes)` reports **27 recipe dirs**. **26** declare `native_octane_verified=true`; **25** carry `octane-preview.png`. **1** unverified: `earth-moon-space` (no preview PNG — live render gap, not metadata). `helicoid-spiral` stays `false` (pixel-QA rejected its preview as blank flat field).
...
- **Current capability direction:** WP10/WP11 landed via practice: `octane_api_corpus_export`, `octane_capabilities`, `octane_probe_types`, live scene harvest, pre-render sanity gates all exposed. WP12 single-source Lua handler generation and WP13 registry-backed material/light compatibility are the highest-leverage bridge-hardening work.

**Maturity read:** 381 green, 1 pre-existing failure, 27 recipes with 26 verified — the project is no longer a prototype queue but a working visual workbench. The docs' status snapshot has drifted 12 commits (the snapshot-rots problem documented in the skill). The active risk is WIP closure: WP12 (single-source Lua), WP13 (registry), and live geo/animation exercises.
