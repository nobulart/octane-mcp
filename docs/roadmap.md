...
## Status snapshot (2026-07-09 — steward run e879a05)

Live-checked 2026-07-09 (autonomous hourly steward, HEAD `e879a05`). Evidence-based, not aspirational — re-grounded fresh this run; the prior block (keyed to `3ad0094`, 230 tests) is stale and replaced below:

- **Repo:** `main` = `e879a05` (HEAD; tree **clean** at steward start). Recent commits: WP8 `octane_build_animation` tool exposed (`e879a05`), WP8 animation DSL model (`1696b38`), WP9 semantic-gate fix (`80915db`), docs TCC target fix (`3ad0094`).
- **Tests:** **234 passed / 5 skipped** (offline `python -m unittest discover -s tests`). Green. 5 skipped (live render; `geo` extra absent ×2; lua executable absent; geo_tool happy-path needs `geo` extra). Up from 230/4 last run — +4 new `swap_geometry` tests. `compileall src` clean; the real launcher (`run_octanex_mcp.sh`, `PYTHONPATH` unset) boots a **48-tool** `build_mcp()` (incl. `octane_flush_queue` + `octane_swap_geometry` added 2026-07-10) with no `benchmarks`/`scripts`/`tests` imports — §6 layering holds (verified via stdio JSON-RPC handshake this run).
...
- **Recipe library — count reconciled via ground truth:** `recipe_index()` + `_recipe_dirs` + `octane-preview.png` presence → **18 recipe dirs, 18 `native_octane_verified=true`** (no remaining gap). `math-surface` was closed in the live session (re-rendered, `octane-preview.png` copied, flag flipped). The "20 dirs / 8 unverified" figure in the oldest snapshot was a scan artifact (non-recipe dirs counted).
...
- **WP8 animation DSL (DONE — model + MCP tool + gateway parity + LIVE DRAIN):** `src/octanex_mcp/animation.py` model + `build_animation_commands()` helper that emits queue-ready `set_camera` + `save_preview` envelopes (relative `renders/frame_XXXX.png` — see path-bug note below). New MCP tool `octane_build_animation` (orbit bake) + gateway dispatch for Canvas parity. Tests: `tests/test_animation.py` (13) + `tests/test_animation_tool.py` (10, all pass). **Live-drained this session**: the bridge consumed the 24-frame orbit queue and wrote `frame_0000..00NN.png` to `OctaneMCP/renders/`; pixel-QA passed; frame-to-frame diff confirmed camera motion (genuine orbit). Path bug found + fixed: `build_animation_commands` must use a *relative* `renders/…` path — an absolute `/…/OctaneMCP/renders/…` is re-based by Octane `saveImage` to the sandbox `Data/renders/`, dropping the `OctaneMCP` segment. Remaining WP8: optional ffmpeg encode step; full 24-frame clip is slow (~2.5 min/frame) because each `save_preview` does a blocking `octane.render.start{}` + wait.
- **Streaming data-grammar primitive (shipped this run):** `src/octanex_mcp/scene.py::swap_geometry` replaces an object's OBJ asset in place while preserving its stable `Hermes::scene::object` node name — the *replaceable-asset-files* half of the north-star streaming protocol (see `docs/canvas-roadmap.md` §2). Reuses the `import_geometry` op (no schema/Lua change); emits a schema-valid `swap_command` envelope (optionally queued). 4 new `SwapGeometryTests`. **Now exposed as `octane_swap_geometry` MCP tool + gateway `DISPATCH` entry (2026-07-10, H2 closed).**
