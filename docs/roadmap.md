...
- **Recipe library — count reconciled:** `recipe_index()` reports **18 recipe dirs, 17 `native_octane_verified=true` (+ reference PNG), 1 unverified** (`math-surface`). `math-surface` is the *intentional* gap: its reference preview PNG was dropped in commit `0993e51` because the recipe has a pre-existing contract gap, so `validate_recipe_library`/`verify_recipe_library` exclude it by design. All other 17 (incl. `avatar-guide`, `data-bars`, `annotated-text-labels`, `architecture-flow`, `document-ocr-layout`) were promoted from real native renders that pass pixel acceptance. The earlier "13/18 / 5 unverified" and "20 dirs / 8 unverified" figures were scan artifacts; the merged reality is 17/18 with `math-surface` as the sole, deliberate gap. |
