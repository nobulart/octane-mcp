...
local function request_render_restart(samples, width, height, do_start, max_render_time)
    ...
    local ok, a, b = pcall(function()
        ...
        for attempt = 1, 5 do
            -- Current live evidence: restart()/continue() return ok but do not
            -- begin sampling (stats stay beauty=0/state=0). start{RT} blocks
            -- until Octane finishes the frame, then saveImage can capture it.
            -- The Python caller must therefore use a drain timeout long enough
            -- for this synchronous render path.
...
local function handle_set_object_transform(cmd)
    ...
    -- Octane GEO mesh transform pins. Use engine constants when present, else
    -- canonical lowercase pin names, so the op is robust across Octane X builds.
