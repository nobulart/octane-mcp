#!/usr/bin/env python3
"""queue_geo_surface.py — queue a single-material GEO heightfield scene.

Mirrors scripts/queue_implicit_surface.py (known-good baseline) but for a
displacement OBJ produced by gen_geo_displacement.py. Earthy palette, oblique
but lower-angle camera (terrain reads better near-grazing than TPMS).
...
No gradients, no bands, no vertex colours (importer/bridge ignore them).
One glossy material, one earthy colour.
"""
...
def main():
    obj_path = sys.argv[1]
    name = sys.argv[2] if len(sys.argv) > 2 else "geo"
    tag = f"hermes_{name}"
    ...
    # Terrain: lower oblique angle than TPMS (rot_x 25 vs 60) so relief reads.
    cam_pos, target = camera_from_bounds(center, radius, rot_x_deg=25.0, rot_z_deg=30.0)
    print(f"bounds center={[round(c, 3) for c in center]} radius={radius:.3f} cam={[round(c, 3) for c in cam_pos]}")

    # Earthy palette for geo surfaces (distinct but terrain-like).
    PALETTE = {
        "egm2008":  [0.55, 0.42, 0.30],
        "elevation":[0.45, 0.38, 0.30],
        "gebco":    [0.30, 0.45, 0.55],
        "geo":      [0.55, 0.42, 0.30],
    }
    color = PALETTE.get(name, PALETTE["geo"])
    ...
    paths.append(write_cmd("create_material", {"name": f"{tag}_mat", "kind": "glossy",
        "color": color, "roughness": 0.5, "metallic": 0.0}))
