"""Pixel-level acceptance checks for benchmark renders — library module.
...
Public entry points:
  * ``evaluate_acceptance(path, criteria) -> (passed, report)`` — benchmark gate.
  * ``reference_to_acceptance(path) -> {acceptance, derived}`` — WP9 pixel-only
    auto-derivation of an acceptance spec from a reference image.
  * ``filter_reference(path) -> {ok, reasons, stats}`` — WP9 harvest gate.

``benchmarks/acceptance.py`` re-exports this module for backward compatibility.
"""
