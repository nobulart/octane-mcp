...
class TestRecipeContractOffline(TestCase):
    ...
    def test_verify_recipe_library_dry_run_counts(self):
        ...
        self.assertEqual(report["total"], 26)
        # 25/26 recipe dirs ship a real reference preview and pass the offline
        # contract (after the ancient-temple merge). `earth-moon-space` is the
        # exception: its scene graph + command sequence are sound, but BOTH live
        # capture attempts (2026-07-10) returned near-empty frames, so the
        # degenerate preview is intentionally NOT committed —— it legitimately
        # fails the offline preview-exists contract until a converged live render
        # is produced.
        self.assertEqual(report["contract_ok"], 25, report)
